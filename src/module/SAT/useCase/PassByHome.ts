...
import { SAT_SELECTORS_CONFIG, SAT_SELECTORS_HOME } from '../entities/SAT.Selectors';
...
export class PassByHome {
  static async execute(context: SatService) {
    ...
    if (!captcha_token) {
      ...
      logger(`Nota: ${context.code} status: ${entityNotes.status} date: ${dateProcessed}`);
    ...
    await context.page.evaluate(insertCaptchaCode, SAT_SELECTORS_HOME, captcha_token);
